...
def sync_docx_directory_to_collection(
    ...
    """Synchronize all .docx files from root_dir into a ChromaDB collection.

    Notes:
        For each file:
        - Deterministic chunk IDs are generated: "<rel_path>::chunk:<idx>";
        - A file_signature (MD5) is computed from the content;
        - If the collection already has chunks with the same source and signature,
          the file is skipped;
        - If the signature has changed, old chunks are deleted and new ones are written.
        - Files removed from disk are also removed from the collection.

    Args:
        logger: Logger instance.
        root_dir: Root directory with .docx files organized into topic subfolders.
        collection_name: Target ChromaDB collection name.
        **kwargs: api_key, folder_id, api_url, host, port.
    """
    ...
    for source, file_chunks in by_source.items():
        ...
        if old_ids:
            ...
            if old_sig == new_signature:
                logger.info(f"  -> unchanged, skipping (signature={old_sig})")
                continue
