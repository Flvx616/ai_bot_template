...
class PostgresClient:
    ...
    def _log_config_info(self) -> None:
        """Log connection configuration (with masked password)."""
        ...
        self.logger.info(
            ...
            f"pool_max_idle={self.settings.pool_max_idle:.1f}s "
        )

    async def ensure_pool(self) -> None:
        """Initialize the connection pool if not yet created.

        Raises:
            psycopg.OperationalError if no connection is available;
            asyncio.TimeoutError if no connection is available in time.
        """
    ...
    async def _take_conn_in_loop(self, dead_connection_count: int, max_attempts: int) -> AsyncConnection | None:
        while dead_connection_count < max_attempts:
            ...
            try:
                ...
                self.logger.debug(f"Alive conn {id(conn)} retrieved after {dead_connection_count} dead connections")
                ...
                self.logger.warning(
                    f"Dead connection {id(conn)} retrieved."
    ...
    async def get_user_checkpointer(self):
        """Async context manager that yields an AsyncPostgresSaver for LangGraph checkpointing."""
        ...
        try:
            ...
            self.logger.debug(f"Postgres.get_user_checkpointer: conn {id(conn)} returned to pool {id(self)}")
    ...
    async def get_pool_stats(self) -> dict[str, Any] | None:
        ...
        stats = self._pool.get_stats()
        self.logger.info(
            f"Postgres pool stats: pool_size={stats.get('pool_size', 0)} "
    ...
    async def close(self) -> None:
        if self._pool:
            ...
            self._pool = None
            self.logger.info(f"Postgres pool closed {id(self)}")
        else:
            self.logger.debug(f"Postgres pool already closed {id(self)}")
