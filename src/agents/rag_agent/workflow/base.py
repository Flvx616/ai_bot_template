...
from agents.rag_agent.states import AgentState, AgentStatus


def build_builder(agent: RagAgent, checkpointer: MemorySaver | AsyncPostgresSaver | None) -> CompiledStateGraph:
    r"""Build the RAG agent graph.

    Notes:
        1. Contains input/output validation nodes based on compliance policy;
        2. Can run a retry loop if the model answer does not satisfy the user question.

    Args:
        checkpointer: Object for persisting agent state between requests;
