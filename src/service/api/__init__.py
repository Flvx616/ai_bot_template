...
def create_app() -> FastAPI:
    app_man = FastAPI(title="RagChatBot", lifespan=lifespan)
    app_man.middleware("http")(log_requests)
    app_man.include_router(service_router, tags=["Service routes"])
    app_man.include_router(metric_router, tags=["Feedback metric routes"])
    app_man.include_router(v1_router, prefix="/api/v1", tags=["v1"])
    return app_man
